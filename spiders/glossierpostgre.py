...
class SpiderModel:
    ...
    def insert(self,data_product,data_review):
        connection = self.connect()
        cur = connection.cursor()
        ...
        print(list(data_product.values()))
        cur.execute("INSERT INTO spider_product_info (TITLE, DESCRIPTION, PRICE, CURRENCY, BRAND, SELLER, IMAGE, CATEGORY, REVIEW_COUNT, REVIEW_RATING, CREATED, LAST_UPDATED, PRODUCT_URL, SPIDER) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",list(data_product.values()))
        connection.commit()
        connection.close()